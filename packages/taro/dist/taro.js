(function(global, factory) {
  ...
  function isObject$1(val) {
    return (
      val != null && _typeof(val) === "object" && Array.isArray(val) === false
    );
  }
  ...
  function getOriginal(item) {
    ...
    if (isObject$2(item)) {
      return (
        item[env === ENV_TYPE.SWAN ? "privateOriginal" : "$original"] || item
      );
    }
    ...
    (function(_Array) {
      ...
      function RefsArray() {
        ...
        _this = _possibleConstructorReturn(
          this,
          (_getPrototypeOf2 = _getPrototypeOf(RefsArray)).call.apply(
            _getPrototypeOf2,
            [this].concat(_toConsumableArray(initList))
          )
        );
        _this.inited = false;
        return _this;
      ...
      _createClass(Link, [
        {
          ...
          value: function request(requestParams) {
            ...
            this.chain.interceptors = this.chain.interceptors.filter(function(
              interceptor
            ) {
  ...
  function usePageLifecycle(callback, lifecycle) {
    ...
    if (!hook.marked) {
      ...
      hook.component[lifecycle] = function() {
        ...
        originalLifecycle &&
          originalLifecycle.call.apply(
            originalLifecycle,
            [component].concat(Array.prototype.slice.call(arguments))
          );
        return (
          callback &&
          callback.call.apply(
  ...
  function useEffectImpl(effect, deps, delay) {
    ...
    if (areDepsChanged(hook.deps, deps)) {
      ...
      if (delay) {
        ...
        Current.current.layoutEffects = Current.current.layoutEffects.concat(
          hook
        );
      }
    }
  ...
  function shallowEqual(obj1, obj2) {
    ...
    for (var i = 0; i < obj1Keys.length; i++) {
      ...
      if (
        ...
        !objectIs$1(obj1[obj1KeyItem], obj2[obj1KeyItem])
      ) {
        return false;
      }
    }
  ...
  function memo(component, propsAreEqual) {
    component.prototype.shouldComponentUpdate = function(nextProps) {
      return isFunction$1(propsAreEqual)
